...
class Midtrans extends REST_Controller {
    ...
    public function index_post($api_key=null)
    {
        ...
        $external_id    = $this->post("external_id");//kemungkinan metadata
        $bank_code    = $this->post("bank_code");
        $xendit_id    = $this->post("callback_virtual_account_id");//kemungkinan order_id
        ...
        if(isset($cara_pembayaran->project_id)){
            ...
            $tagihan_air = $this->db->select("
								...
								WHEN v_tagihan_air.periode > '$periode' THEN 0
									...
									END 
                                END,0),'|',                                
								isnull(CONVERT(int,v_pemutihan.nilai_tagihan),-1),'|', 
								isnull(CONVERT(int,v_pemutihan.nilai_denda),-1)) as tagihan")
            ...
            $tagihan_lingkungan = $this->db->select("
								...
								isnull(CASE
									...
									ELSE
										...
										END 
									END,0),'|',0,'|',
                                isnull(CONVERT(int,v_pemutihan.nilai_tagihan),0),'|',
								isnull(CONVERT(int,v_pemutihan.nilai_denda),0)) as tagihan")	
            ...
            // $tagihanTMP     = $this->db
            //                     ->select("
            //                             v_sales_force_pending.*,
            //                             isnull(t_tagihan_lingkungan.id,0) as lingkungan_id,
            //                             isnull(t_tagihan_air.id,0) as air_id,
            //                             isnull(service_air.id,0) as service_air_id,
            //                             isnull(service_lingkungan.id,0) as service_lingkungan_id,
            //                             isnull(
            //                                 CASE
            //                                     WHEN v_tagihan_air.nilai_denda_flag = 1 THEN v_tagihan_air.nilai_denda 
            //                                     WHEN v_tagihan_air.periode > '".$periode."' THEN 0
            ...
            //                                         WHEN v_tagihan_air.denda_jenis_service = 2 THEN v_tagihan_air.denda_nilai_service *(DateDiff( MONTH, v_tagihan_air.periode, '".$periode."' ) + IIF(".date("d").">=service_air.denda_tanggal_jt,1,0) )
            //                                         WHEN v_tagihan_air.denda_jenis_service = 3 THEN (v_tagihan_air.denda_nilai_service* v_tagihan_air.total/ 100 ) *(DateDiff( MONTH, v_tagihan_air.periode, '$periode' ) + IIF(".date("d").">=service_air.denda_tanggal_jt,1,0) )
            //                                     END 
            //                                     END,0) AS denda_air,
            //                                     isnull(
            //                                         CASE
            ...
            //                                             ELSE
            //                                                 CASE
            //                                                     WHEN service_lingkungan.denda_flag = 0 THEN 0
            //                                                     WHEN v_tagihan_lingkungan.denda_jenis_service = 1 
            //                                                         THEN v_tagihan_lingkungan.denda_nilai_service 
            //                                                     WHEN v_tagihan_lingkungan.denda_jenis_service = 2 
            //                                                         THEN 
            //                                                             v_tagihan_lingkungan.denda_nilai_service * 
            //                                                                 (DateDiff( MONTH, DATEADD(month,service_lingkungan.denda_selisih_bulan,v_tagihan_lingkungan.periode), '$periode' ) + IIF(".date("d").">=service_lingkungan.denda_tanggal_jt,1,0) )
            //                                                     WHEN v_tagihan_lingkungan.denda_jenis_service = 3 THEN 
            //                                                     ( ( v_tagihan_lingkungan.denda_nilai_service* v_tagihan_lingkungan.total/ 100 ) * DateDiff( MONTH, v_tagihan_lingkungan.periode, '$periode' ))
            //                                                 END 
            //                                             END,0) AS denda_lingkungan")
            //                     ->from($from."v_sales_force_pending")
            //                     ->join($from."t_tagihan_lingkungan",
            //                             "t_tagihan_lingkungan.t_tagihan_id = v_sales_force_pending.bill_id
            //                             AND t_tagihan_lingkungan.status_tagihan != 1",
            //                             "LEFT")
            //                     ->join($from."v_tagihan_lingkungan",
            //                             "v_tagihan_lingkungan.tagihan_id = v_sales_force_pending.bill_id",
            //                             "LEFT")
            //                     ->join($from."t_tagihan_air",
            //                             "t_tagihan_air.t_tagihan_id = v_sales_force_pending.bill_id
            //                             AND t_tagihan_air.status_tagihan != 1",
            //                             "LEFT")
            //                     ->join($from."v_tagihan_air",
            //                             "v_tagihan_air.tagihan_id = v_sales_force_pending.bill_id",
            //                             "LEFT")
            //                     ->join($from."service as service_lingkungan",
            //                             "service_lingkungan.service_jenis_id = 1
            //                             AND service_lingkungan.active = 1
            //                             AND service_lingkungan.project_id = t_tagihan_lingkungan.proyek_id",
            //                             "LEFT")
            //                     ->join($from."service as service_air",
            //                             "service_air.service_jenis_id = 2
            //                             AND service_air.active = 1
            //                             AND service_air.project_id = t_tagihan_air.proyek_id",
            //                             "LEFT")
            ...
            $success = 0;
            // $tagihan = [];
            // foreach ($tagihanTMP as $v) {
            //     if($v->lingkungan_id != 0){
            //         $tmp = "$v->service_lingkungan_id|1|$v->lingkungan_id|$v->tagihan_lingkungan|$v->denda_lingkungan|0|0|0";
            //         array_push($tagihan,$tmp);
            //     }
            //     if($v->air_id != 0){
            //         $tmp = "$v->service_air_id|2|$v->air_id|$v->tagihan_air|$v->denda_air|0|0|0";
            //         array_push($tagihan,$tmp);
            //     }
            // }

            // var_dump($tagihan);
            // v.service_id + "|" + v.service_jenis_id + "|" + v.tagihan_id + "|" + v.nilai_tagihan + "|" + v.nilai_denda + "|" + 0 + "|" + v.view_pemutihan_nilai_tagihan + "|" + v.view_pemutihan_nilai_denda
            // $tagihan_lingkungan = $this->db->select("
            //                     CONCAT(service.id,'|',service.service_jenis_id,'|',t_tagihan.id,'|',v_tagihan_lingkungan.total,'|',
            //                     isnull(CASE
            //                     WHEN v_tagihan_lingkungan.nilai_denda_flag = 1 THEN v_tagihan_lingkungan.nilai_denda +total
            //                     WHEN v_tagihan_lingkungan.periode >= GETDATE() THEN 0+total
            //                     ELSE
            //                     CASE
            ...
            //                             v_tagihan_lingkungan.denda_nilai_service * (DateDiff( MONTH, DATEADD(month,service.denda_selisih_bulan,v_tagihan_lingkungan.periode), GETDATE() ) + IIF(DAY(GETDATE())>=service.denda_tanggal_jt,1,0) )
            //                         WHEN v_tagihan_lingkungan.denda_jenis_service = 3 THEN
            //                             ( v_tagihan_lingkungan.denda_nilai_service* v_tagihan_lingkungan.total/ 100 ) * DateDiff( MONTH, v_tagihan_lingkungan.periode, GETDATE() )
            //                     END 
            //                     END,0),'|0|0|0') as tagihan_lingkungan")
            //             ->from("t_tagihan")
            ...
            //                     AND service.project_id = t_tagihan.proyek_id")
            //             ->where_in("t_tagihan.id",$tagihan)
            //             ->get()->result();
            // echo("yuhuu");
            // var_dump($tagihan);
            ...
            $keySF = json_decode($this->curl->simple_post("https://test.salesforce.com/services/oauth2/token",$dataApi));
            // echo("key");
            // var_dump($keySF);
            ...
            if($checkout_data->salesforce_id){
                $data = [
                    "Status__c"     => "Paid",
                    "Paid_Date__c"  => date("Y-m-d")."T".date("H:i:s.000")."Z"
                ];  
