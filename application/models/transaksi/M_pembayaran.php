			...
			foreach ($bayarTMP as $k=>$v) {
				...
				if ($tmp[3] + $tmp[4] + $tmp[5] ) {
					...
					array_push($pembayaran_detail, [
						...
						"nilai_ppn" => $this->db->select("isnull(ppn_flag,0) as ppn_flag")->from("service")->where("id",$tmp[0])->get()->row()->ppn_flag?$nilai_ppn:0
					]);
					...
					if ($tmp[1] == 1) {
						...
						// $this->db->join('t_tagihan_loi',
						// 				"t_tagihan_loi.loi_id = t_tagihan_loi.id
						// 				 t_tagihan_loi.tipe = 1");
						// $this->db->where('t_tagihan_loi.id', $tmp[2]);

						// $this->db->update('t_loi_registrasi')
						// 		  ->set('status_bayar',1)
						// 		  ->;
						...
						$sql = "UPDATE t_loi_registrasi 
								...
								JOIN t_tagihan_loi 
									ON t_tagihan_loi.loi_id = t_loi_registrasi.id 
