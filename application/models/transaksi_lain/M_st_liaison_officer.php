...
class m_st_liaison_officer extends CI_Model
{
    public function get()
    {
        $query = $this->db->query('
    ...
    public function save($dataTmp)
    {
        ...
        [  
            ...
            'hasil_aktifasi' => $dataTmp['hasil_aktifasi'],
            'delete' => 0,
        ];
        $this->db->insert('t_loi_aktifasi', $dataAktifasi);
        $id = $this->db->insert_id();

        $dataLog = $this->get_log($id);
        $this->m_log->log_save('t_loi_aktifasi', $id, 'Tambah', $dataLog);
