...
class m_survei_liaison_officer extends CI_Model
{
    public function get()
    {
        $query = $this->db->query('
    ...
    {   
        $validasi1 = $this->db->select("*")
                    ->from("t_loi_survei")
                    ->where("t_loi_registrasi_id",$this->input->get('id'))
                    ->count_all_results();
        if($validasi1 == 0){
            $this->load->model('m_core');
            $this->load->model('m_log');
            $project = $this->m_core->project();
            $tanggal_survei =  $dataTmp['tanggal_survei'];
            $oldDate1 = $tanggal_survei;
            $arr = explode('-', $oldDate1);
            //  $newDate = $arr[2].'-'.$arr[1].'-'.$arr[0];
            $tanggal_survei2 = $arr[2].'-'.$arr[1].'-'.$arr[0];
            
            if($dataTmp['nama_item'] == null){
                $dataRegister =
                [
                    'nomor_registrasi' => $dataTmp['nomor_registrasi'],
                    'status_item_charge' => 0,
                    'status_dokumen' => 3
                ];
            }else{
                $dataRegister =
                [
                    'nomor_registrasi' => $dataTmp['nomor_registrasi'],
                    'status_item_charge' => 1,
                    'status_dokumen' => 2
                ];
            }
            $hasil_survei = ($dataTmp['hasil_survei']=='on')?1:0;
     
            if ($this->db->count_all_results() != 0 && $hasil_survei != 0) {
                ...
                if ($this->db->count_all_results() == 0) {
                    ...
                    [  
                        ...
                        'tanggal_survei' => $tanggal_survei2,
                        'keterangan_survei' => $dataTmp['keterangan_survei'],
                        'hasil_survei' => $hasil_survei,
                        'delete' => 0,
                    ];
                    $this->db->insert('t_loi_survei', $dataSurvei);
                    $id = $this->db->insert_id();
                    if (!empty($dataTmp['nama_item']))
                    {
                        for($i= 0;$i<count($dataTmp['nama_item']);$i++) {            
                            if($dataTmp['nama_item'][$i] != ""){
                                $nama_item[$i]  =  $dataTmp['nama_item'][$i];  
                                $volume[$i] =  $this->m_core->currency_to_number($dataTmp['volume'][$i]);
                                $satuan[$i] = $dataTmp['satuan'][$i];
                                $bukti_survei[$i] = (count($dataTmp['bukti_survei'])=='')?NULL:$dataTmp['bukti_survei'][$i];
                                $dataItem = [
                                    't_loi_registrasi_id'   => $dataTmp['id'],
                                    'nama_item'             => $nama_item[$i],
                                    'volume'                => $volume[$i],
                                    'satuan'                => $satuan[$i],
                                    'bukti_survei'          => $bukti_survei[$i]
                                ];
                                $this->db->insert('item_charge', $dataItem);
                            }
                        }
                    }
                    $dataLog = $this->get_log($id);
                    $this->m_log->log_save('t_loi_survei', $id, 'Tambah', $dataLog);
                }
                return 'success';
            }else{
                return 'pending';
            ...
            redirect(site_url()."/transaksi_lain/p_survei_liaison_officer/");
        }
        return 'success';
    }
    ...
    public function save2($dataTmp)
    {
        ...
        $this->load->model('m_log');
        $project = $this->m_core->project();
        $tanggal_survei =  $dataTmp['tanggal_survei2'];
        $oldDate1 = $tanggal_survei;
        $arr = explode('-', $oldDate1);
        //  $newDate = $arr[2].'-'.$arr[1].'-'.$arr[0];
        $tanggal_survei2 = $arr[2].'-'.$arr[1].'-'.$arr[0];
        ...
        if ($this->db->count_all_results() != 0) {
            ...
            if ($this->db->count_all_results() == 0) {
                ...
                $dataSurvei =
                [  
                    'tanggal_survei2' => $tanggal_survei2,
                    'keterangan_survei2' => $dataTmp['keterangan_survei2'],
                    'hasil_survei2' => $dataTmp['hasil_survei2'],
                    'delete' => 0,
                ];
                $this->db->where('t_loi_registrasi_id', $dataTmp['id']);
                $this->db->update('t_loi_survei', $dataSurvei);
                if (empty($dataTmp['nama_item']))
                {
                    for($i= 0;$i<count($dataTmp['nama_item']);$i++) {            
                        $nama_item[$i]  =  $dataTmp['nama_item'][$i];  
                        $volume[$i] =  $this->m_core->currency_to_number($dataTmp['volume'][$i]);
                        $satuan[$i] = $dataTmp['satuan'][$i];
                        $bukti_survei[$i] = $dataTmp['bukti_survei'][$i];
                        $dataItem = [
                            't_loi_registrasi_id'   => $dataTmp['id'],
                            'nama_item'             => $nama_item[$i],
                            'volume'                => $volume[$i],
                        ...
                        $this->db->insert('item_charge_tambah', $dataItem);
                    }
                }
            }
        }
    ...
    public function getAll()
    {
        ...
        $query = $this->db
                            ...
                            ->where("t_loi_registrasi.status_dokumen >=1")
                            ->where("t_loi_registrasi.status_bayar >= 1");
                            // ->or_where("t_loi_registrasi.status_dokumen >= 2");
        return $query->get()->result_array();
    }
